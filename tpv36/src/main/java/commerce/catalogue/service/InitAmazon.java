...
public class InitAmazon {
	...
	public void init() {
		...
		search.setCategory(this.type);	//Books Music
			...
			if (espaceNom != null && !racine.getName().equals("ItemSearchErrorResponse")) {
				...
				while (itemIterator.hasNext() && i != 5) {
					...
					try 
					{
						if (itemAttributes.getChild("ProductGroup",espaceNom).getText().equals("Music")) {
						...
						} else if (itemAttributes.getChild("ProductGroup",espaceNom).getText().equals("DVD")) {
							dvd = new Dvd();
							...
							dvd.setEAN(itemAttributes.getChild("EAN",espaceNom).getText());
